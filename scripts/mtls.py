...
from ssl import OP_NO_TLSv1, OP_NO_TLSv1_1, OP_NO_TLSv1_2, PROTOCOL_TLS_CLIENT, TLSVersion
...
def parse_args():
    ...
    parser.add_argument('-s', '--hosts', default="172.41.0.2", help='Comma-separated ScyllaDB node Names or IPs')
...
def _build_cluster_and_session(hosts, username, password, dc, local_loopback, shard_aware=False ):
    ...
    ssl_context.load_verify_locations('./config/ca/ca.crt')  # Server CA
    ssl_context.verify_mode = CERT_REQUIRED
    ssl_context.check_hostname = False  # Disable hostname checking for IP-based connections
    ssl_context.load_cert_chain(certfile='./config/db/db.crt', keyfile='./config/db/db.key')

    #cluster = Cluster(['scylla-node:9142'], ssl_context=ssl_context)
